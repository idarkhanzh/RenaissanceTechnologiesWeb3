...
class RoostooClient:
    def __init__(self,
        ...
        if not self.api_key or not self.secret:
            print("[RoostooClient] WARNING: API key/secret not set.")

        # default lot sizes (can be refined if you know exact ExchangeInfo)
        self.base_lot_sizes = {
            "BTC":  0.0001,
            "ETH":  0.001,
            "ARB":  0.1,
    ...
    def _sign(self, params: dict) -> str:
        # Roostoo expects sorted query-string style
    ...
    def _round_step(self, base_symbol: str, qty: float) -> float:
        """
        Round qty DOWN to nearest allowed lot size.
        Returns 0 if below one step or invalid.
        """
        base = base_symbol.upper()
        step = float(self.base_lot_sizes.get(base, 0.01))  # fallback 0.01
        if qty <= 0 or step <= 0:
            return 0.0
        units = math.floor(qty / step)
        if units <= 0:
            return 0.0
        return float(f"{units * step:.8f}")
    ...
    def place_market_order(self,
        ...
        if qty is None or qty <= 0:
            print(f"[RoostooClient] SKIP {base} {side}: non-positive qty {qty}")
            return None

        adj_qty = self._round_step(base, float(qty))
        if adj_qty <= 0:
            print(f"[RoostooClient] SKIP {base} {side}: qty {qty} -> {adj_qty} below step")
            return None
        ...
        payload = {
            ...
            "side": side,
            "type": "MARKET",
            "quantity": f"{adj_qty}",  # API docs: STRING
        }
        ...
        try:
            ...
            print(f"[RoostooClient] HTTP ERROR {base} {side} {adj_qty}: {e}")
            return None

        if not isinstance(resp, dict) or not resp.get("Success", False):
            print(f"[RoostooClient] ROOSTOO ERROR {base} {side} {adj_qty}: {resp}")
            return None

        print(f"[RoostooClient] ORDER OK {base} {side} {adj_qty}: {resp}")
        # optional: append_trade(base, side, adj_qty, price or resp price)
        return resp
    ...
    while True:
        try:
            ...
            for base in bases:
                ...
                if diff_val > 0:
                    # BUY: can't spend more than quote_balance
                    ...
                    if qty <= 0:
                        continue
                    side = "BUY"
                    quote_balance -= buy_val  # update local cash snapshot

                else:
                    # SELL: long-only -> never sell more than we own
                    desired_sell_val = -diff_val
                    max_sell_val = cur_val
                ...
                roostoo.place_market_order(base, side, qty, px)
